...
- **Refused subjects.** The tenants and API keys with at least one refusal in the last hour, top five each. Tenants are shown by slug and keys by label (or public prefix). A subject the gateway cannot resolve is shown as "unknown tenant" or "unknown key", never by its id. Anonymous callers are shown as "anonymous"; their client address block is not included in this section. A key-scoped limit is named the same way (key label or prefix, then the model). A count followed by `+` means the report hit its row cap, so it is a lower bound.
