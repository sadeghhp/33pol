...
| ID | Title | Notes |
|---|---|---|
| **RL-021** | ~~With authentication disabled, every authorization policy including `Operator` succeeds, so `PUT /admin/api/rate-limits` — including the `enabled` master switch — is reachable with no credential.~~ **Fixed — see GW-01.** | This was recorded here as an explicit mode rather than a defect, on the reasoning that `GatewayAuthenticationInitializer` fails closed outside Development. It did not: the initializer was registered only on the branch taken when a connection string *was* configured, so the guard could never run in the mode it was written for, and a Production host shipping the default empty connection string served the rate-limit control plane — the `enabled` master switch included — to anonymous callers. Both halves are now fixed: registration refuses that configuration before Kestrel binds, and `GatewayAuthorizationHandler` no longer succeeds `Admin` or `Operator` for an anonymous caller in any mode. `Gateway:Security:AllowAnonymous=true` still opts into anonymous **inference**; it does not open the control plane. See [security.md](security.md#running-without-authentication). |
