...
install_build_env_content() {
...

  {
    ...
    # The gateway uses an embedded SQLite database persisted on the gateway-data volume
    # (compose defaults GatewayDb to Data Source=/data/gateway.db). No external DB to configure;
    # set GATEWAY_DB_CONNECTION in an override only to relocate the file.
