...
public sealed class SqliteDailyUsageRollupConcurrencyTests
{
    ...
    /// serializable transaction, which Microsoft.Data.Sqlite issues as <c>BEGIN IMMEDIATE</c>. That
    /// takes the write lock before the read, so a second writer cannot observe a stale starting
    /// value and overwrite the first writer's usage.
    /// </summary>
    /// <remarks>
